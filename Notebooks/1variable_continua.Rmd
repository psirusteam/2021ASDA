...
```{r}
diseno %>% group_by(Sex) %>%
  summarise(Media = survey_mean(Expenditure, level = 0.95,
                         vartype =  c("se", "ci")))
...
## Estimación de la proporción de hombres

```{r}
diseno %>% mutate(Hombres = (Sex == "Male")) %>%
  summarise(
    Prop = survey_mean(Hombres, level = 0.95,
                       vartype =  c("se", "ci"))) %>%
  data.frame()
```

## Estimación de la proporción de hombres en la zona rural

```{r}
sub_Rural %>% mutate(Hombres = (Sex == "Male")) %>%
  summarise(
    Prop = survey_mean(Hombres, level = 0.95,
                       vartype =  c("se", "ci"))) %>%
  data.frame()
...
## Estimación de la proporción de mujeres menor de 18 años

```{r}
sub_Mujer %>% mutate(Menor_18 = Age < 18) %>%
  summarise(
    Prop = survey_mean(Menor_18, level = 0.95,
                       vartype =  c("se", "ci"))) %>%
  data.frame()
```

## Estimación de la varianza y desviación estándar de los ingresos

```{r}
(tab_var <- diseno %>% summarise(
    Var = survey_var(Income, level = 0.95,
                         vartype =  c("se", "ci"), 
                       deff = TRUE)))
sqrt(tab_var)
...
```{r}
(prom_region <- svyby(~Income, ~Region, diseno, svymean, na.rm=T, covmat = TRUE))
...
## contrastes
```{r}
svycontrast(prom_region, list(diff_NS=c(1,-1,0,0,0))) %>% 
  data.frame()
...
```{r}
prom_sex <- svyby(~Income, ~Sex, diseno, svymean, na.rm=T,covmat = TRUE)
svycontrast(prom_sex, list(diff_Sexo = c(1,-1)))
...
## Contrastes no independiente
```{r}
sum_region <- svyby(~Income, ~Region, diseno, svytotal, na.rm=T,covmat = TRUE)
...
svycor( ~ Income + Expenditure, design = diseno)$cors %>% data.frame()
svycor( ~ Income + Expenditure, design = sub_Hombre)$cors %>% data.frame()
svycor( ~ Income + Expenditure, design = sub_Mujer)$cors %>% data.frame()
