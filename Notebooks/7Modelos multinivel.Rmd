...
ggplot(data = encuesta_plot, aes(y = Income, x = Expenditure)) + 
  geom_jitter() +
...
## Ajuste de pesos (alternativa a los Modelo q-weighted) 
```{r, echo=TRUE,eval=TRUE}
mod_qw <- lm(wk ~ Age + Sex + Region + Zone,
             data = encuesta)
encuesta$wk2 <-   encuesta$wk/predict(mod_qw)
# alternativa los q-weighted
n = nrow(encuesta)
...
## Modelo con intercepto aleatoria 
```{r}
coef(mod_Int_Aleatorio)$Stratum %>% slice(1:10L)
```
...
```{r}
(tab_pred <- data.frame(Pred = predict(mod_Int_Aleatorio), 
           Income = encuesta$Income, 
           Stratum = encuesta$Stratum)) %>% distinct() %>% 
  slice(1:6L) # Son las pendientes aleatorias
```
...
logístico: 
$$
Pr\left(y_{ij}\right)=Pr\left(y_{ij}=1\mid x_{i}:\boldsymbol{\beta}\right)=\frac{\exp\left(\boldsymbol{\beta}_{j}\boldsymbol{x}_{ij}\right)}{1-\exp\left(\boldsymbol{\beta}_{j}\boldsymbol{x}_{ij}\right)}
$$ 
ó
...
## Estimación de la propoción total con $y$ y $\hat{y}$
